...
static void QuadraticEquation(int* ptrPolynomial, int multiplicity)
{
  ...
  for (ctr = 0; ctr < 2; ctr++)
  {
    ...
    *ptrOutput++ = ' ';
    ...
    ShowRationalAndSqrParts(&Rat2, &Rat3, 2, pretty, ptrTimes);
    ...
    for (ctr = 0; ctr < 4; ctr++)
    {
      ...
      {  // x = +/- sqrt((sqrt(e) - c) / 2) +/- i * sqrt((sqrt(e) + c) / 2)
        ...
        for (ctr2 = 0; ctr2 < 2; ctr2++)
        {
          ...
          {
            ShowRationalAndSqrParts(&Rat1, &Rat2, 4, pretty, ptrTimes);
          }
          else
          {
            startSqrt();
      ...
      {  // x = sqrt(-c/2 +/- sqrt(c^2 - 4e)/2)
        ...
        if (BigIntIsZero(&RatDeprQuadratic.numerator))
        {
          ...
          showSquareRootOfRational(&RatDeprIndependent, 4, pretty, ptrTimes);
          BigIntChSign(&RatDeprIndependent.numerator);
          showText("</li>");
        }
        else
...
static void showSquareRootOfComplex(char* plus, char* minus)
{
  ...
  showSquareRootOfRational(&Rat4, 2, pretty, ptrTimes);
...
static void FerrariResolventHasRationalRoot(int multiplicity)
{
  ...
  for (ctr = 0; ctr < 4; ctr++)
  {
    ...
    *ptrOutput++ = ' ';
    ...
    {             // S is real.
      int isImaginary;
      showSquareRootOfRational(&RatS, 2, pretty, ptrTimes);
      if (ctr == 0 || ctr == 1)
...
static void QuarticEquation(int* ptrPolynomial, int multiplicity)
{
  ...
  else if (RatDiscr.numerator.sign == SIGN_POSITIVE)
  {
    ...
    Rat2.numerator.sign = SIGN_POSITIVE;
    showRationalOverStr(&Rat2, pretty, "Q", ptrTimes);
    endSqrt();
    showText("</li>");
    ...
    for (ctr = 0; ctr < 4; ctr++)
    {
      ...
      *ptrOutput++ = ' ';
      ...
      if (!BigIntIsZero(&RatDeprLinear.numerator))
      {
        ...
        showRationalOverStr(&RatDeprLinear, pretty, "S", ptrTimes);
      }
      endSqrt();
    }
  }
    ...
    for (ctr = 0; ctr < 2; ctr++)
    {
      ...
      for (component = 0; component < 2; component++)
      {
        ...
        *ptrOutput++ = ' ';
        ...
        ShowRationalAndSqrParts(&Rat5, &Rat3, 2, pretty, ptrTimes);
        if (!pretty)
        {
          *ptrOutput++ = ')';
        }
