...
static void showBlocklyError(int rc)
{
  ...
  case BLOCKLY_TOO_MANY_VARIABLES:
    formatString(&ptrTemp, "K$1s", LITERAL_SHOW_BLOCKLY_ERROR1); // Too many variables defined.
    break;
  case BLOCKLY_VARIABLE_NAME_TOO_LONG:
    formatString(&ptrTemp, "K$1s", LITERAL_SHOW_BLOCKLY_ERROR2); // Variable name is too long.
    break;
  case BLOCKLY_ONE_TOP_BLOCK:
    formatString(&ptrTemp, "K$1s", LITERAL_SHOW_BLOCKLY_ERROR3); // There must be only one set of joined blocks.
    break;
  case BLOCKLY_INVALID_BLOCK_TYPE:
    formatString(&ptrTemp, "K$1s", LITERAL_SHOW_BLOCKLY_ERROR4); // Invalid block type.
    break;
  case BLOCKLY_INVALID_NAME_FOR_SHADOW:
    formatString(&ptrTemp, "K$1s", LITERAL_SHOW_BLOCKLY_ERROR5); // Invalid name for shadow tag in XML.
    break;
  case BLOCKLY_TOO_MANY_CLOSING_BLOCK_TAGS:
    formatString(&ptrTemp, "K$1s", LITERAL_SHOW_BLOCKLY_ERROR6); // Too many /block tags.
    break;
  case BLOCKLY_NO_OUTPUT_EXPECTED:
    formatString(&ptrTemp, "K$1s", LITERAL_SHOW_BLOCKLY_ERROR7); // The detached block must be inside another block.
    break;
  case BLOCKLY_CANNOT_PARSE_XML:
    formatString(&ptrTemp, "K$1s", LITERAL_SHOW_BLOCKLY_ERROR8); // Internal Error: cannot analyze XML generated by Blockly.
    break;
  default:
    return;
  }
  SEND_DATA_TO_OUTPUT(temp);
}
...
void fromBlockly(const char* ptrXMLFromBlockly)
{
  ...
  if (ComputeExpression(bufferInstr, NULL) ==
    EXPR_SYNTAX_ERROR)
  {
    formatString(&ptrBlocklyOutput, "<li>$1s</li>", LITERAL_FROM_BLOCKLY1); // Internal error.
  }
  else
  {
    if (nbrBlocklyOutputLines == 0)
    {
      formatString(&ptrBlocklyOutput, "<li>$1s</li>", LITERAL_FROM_BLOCKLY2); // There is nothing to print.
    }
  }
  copyStr(&ptrBlocklyOutput, "</ul>");
  beginLine(&ptrBlocklyOutput);
  showCopyright(&ptrBlocklyOutput);
